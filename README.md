...
1. 在指定日期窗口内抓取最多 300 篇候选论文。
2. 合并 arXiv 与受关注机构的官方论文渠道，并完成去重和粗排。
3. 先对全部候选做领域与质量初筛，再对最多 30 篇读取正文。
4. 依据方法、数据、实验、消融、真机验证和可复现性筛选最多 15 篇；质量不足时允许少选。
...
- 按 arXiv ID、规范化标题、作者和项目页去重，候选池上限为 300 篇。
