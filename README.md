...
1. `paper-daily` 脚本负责从 arXiv（以及默认关闭的 OpenReview / OpenAlex）抓取候选论文、统一元数据、去重和粗排序。
2. Codex 作为科研助理读取候选池，并补查受关注公司的官方研究页、项目页、GitHub 与 Hugging Face；所有候选合并去重后再做语义评分、全文核验与日报撰写。
...
- 以 arXiv 为主抓取近期论文候选；同时补查受关注公司的官方渠道，以发现尚未进入 arXiv 的完整论文或技术报告。
...
![Codex 自动化用法](paper-daily/tutorial/codex%20automation.png)
