...
test("renders public report, database, paper, and company routes", async () => {
  const [archive, latestDetail, latestPaper, officialPaper, detail, database, paper, contactPaper, excludedPaper, navigationPaper, traversabilityPaper, memoryPaper, companies, removedAbout] =
    await Promise.all([
    render("/reports"),
    render("/reports/2026-08-14"),
    render("/papers/2608.10232"),
  ...
  assert.match(latestDetailHtml, /REPORT (?:<!-- -->)?2026\/08\/14/);
  assert.match(latestDetailHtml, /FACT: Failure-Aware Causal Training/);
  assert.match(latestDetailHtml, /ContactGuard/);
  assert.match(latestDetailHtml, /82%/);
  assert.doesNotMatch(latestDetailHtml, /SpeedTuning/);
  assert.doesNotMatch(latestDetailHtml, /综合分|候选论文|内部评分/);
  assert.match(latestPaperHtml, /University of California San Diego/);
  assert.match(latestPaperHtml, /Failure Data/);
  ...
  assert.match(companiesHtml, /最近检查：(?:<!-- -->)?2026\.08\.10/);
